...
export function useGraph<T>(
  ...
  const { scope } = { scope: ["User.Read", "User.Read.All", "Group.Read.All"], ...options };
  ...
  const { data, error, loading, reload } = useData(
    async () => {
      try {
        ...
        const userCredential = await teamsfx.getCredential();
        console.log(userCredential);
        // if(userCredential === undefined || (userCredential !== undefined && !userCredential.initialized)) {
