...
export default async function run(
  ...
  try {
    ...
    if (currentUser && currentUser.displayName) {
      ...
      let graph = createMicrosoftGraphClient(teamsfx, [".default"]);
      ...
      let content = {
        ...
        roles: [],
        "user@odata.bind":
          "https://graph.microsoft.com/v1.0/users('" + currentUser.objectId + "')",
      };
      if(teamId !== "") {
        let url = "/teams/" + teamId + "/members";
        let team = await graph
          .api(url)
          .post(content, (error, response, rawResponse) => {
            console.log(response);
            console.log(error);
            if(error === null && response !== null && response.userId === currentUser.objectId) {
              res.body.userId = response.userId;
              res.body.status = 200;
              return res;
            } else {
              res.body.userId = response.userId;
